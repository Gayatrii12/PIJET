    ...
    <div className="z-10 sticky  top-0 md:relative">
      ...
      <div className="navbar-wrapper absolute  md:relative items-center text-white text-xl justify-center- md:h-auto w-3/5  md:w-full rounded-lg md:rounded-none my-1 mx-2 md:my-0 md:mx-0 p-15 shadow-xl md:shadow-md sm:text-xs  bg-pijet-blue px-10 ">
        <div className={" md:h-auto "}>
          <ul
            className={
              " flex-col space-y-4 md:space-y-0 z-10  md:flex-row md:flex items-center py-5 md:py-3 justify-evenly text-white " +
              (isOpen ? "flex" : "hidden")
            }
...
const NavItem = ({ to, active, children }) => {
  return (
    <Link to={to}>
      <motion.li
        className={`font-manrope font-medium   lg:text-xl ${active ? 'text-white' : 'text-white'}`}
        whileTap={{ scale: 0.9 }}
      ...
      >
        {children}
        {active && <motion.hr className="border-b-2 rounded-sm border-white mt-1 " initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring",delay: 0 }} />}
      </motion.li>
    </Link>
  );
};
